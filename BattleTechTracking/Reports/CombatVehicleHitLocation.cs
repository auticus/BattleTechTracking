...
{
    public class CombatVehicleHitLocation : BaseChart
    {
        private const int FULL_COL_SPAN = 4;

        public CombatVehicleHitLocation()
        {
            LoadEntries();
        }
        ...
        public override Grid GenerateChart()
        {
            ...
            InjectChartHeader(grid, "COMBAT VEHICLE HIT LOCATION TABLE", FULL_COL_SPAN, Color.Gold);
        ...
        private void LoadEntries()
        {
            ...
            ChartEntries.Add(new[] { "3", "Front**", "Rear**", "Side**" });
            ChartEntries.Add(new[] { "4", "Front**", "Rear**", "Side**" });
            ...
            ChartEntries.Add(new[] { "10", "Turret", "Turret", "Turret" });
            ChartEntries.Add(new[] { "11", "Turret", "Turret", "Turret" });
            ChartEntries.Add(new[] { "12", "Turret*", "Turret*", "Turret*" });
        ...
        {
            var row = 13;
            InjectLabelInFooter(grid, "*Apply damage normal then roll on crit hits table.  If no turret, then crit hit the side.",
                row++,
                Color.Gold,
                FULL_COL_SPAN);

            InjectLabelInFooter(grid, "**Roll on Motive Damage Table",
                row,
                Color.Gold,
                FULL_COL_SPAN);
        }
